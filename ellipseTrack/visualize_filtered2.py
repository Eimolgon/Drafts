...
def plot_comparison(tracking_data, filtered_data):
    """Plot raw and filtered data comparison with specified colors."""
    plt.figure(figsize=(15, 10))
    
    # Define color scheme
    raw_colors = ['blue', 'orange']  # For class 0 and 1
    filtered_colors = ['green', 'red']  # For class 0 and 1
    
    # Plot X Position
    ax1 = plt.subplot(2, 2, 1)
    for class_id, data in tracking_data.items():
        color_idx = class_id % len(raw_colors)
        if class_id == 0:
            class_name = 'Front wheel'
        elif class_id == 1:
            class_name = 'Rear wheel'

        ax1.plot(data['frames'], data['x_pos'], '-', color=raw_colors[color_idx], 
                alpha=0.5, label=f'Raw {class_name}')
        ax1.plot(filtered_data[class_id]['frames'], filtered_data[class_id]['x_pos'], '-', 
                color=filtered_colors[color_idx], linewidth=2, label=f'Filtered {class_name}')
    ...
    for class_id, data in tracking_data.items():
        ...
        ax2.plot(data['frames'], data['y_pos'], '-', color=raw_colors[color_idx], alpha=0.5)
        ax2.plot(filtered_data[class_id]['frames'], filtered_data[class_id]['y_pos'], '-', 
                color=filtered_colors[color_idx], linewidth=2)
    ...
    for class_id, data in tracking_data.items():
        color_idx = class_id % len(raw_colors)
        ax3.plot(data['frames'], data['major_axes'], '-', color=raw_colors[color_idx], alpha=0.5)
    ...
    for class_id, data in tracking_data.items():
        color_idx = class_id % len(raw_colors)
        ax4.plot(data['frames'], data['angles'], '-', color=raw_colors[color_idx], alpha=0.5)
