...
def calculate_differences_and_slopes(filtered_data, absolute=True, window_size=5):
    """Calculate differences and their slopes between both ellipses."""
    ...
    if len(x_diff) > window_size:
        dx = np.diff(x_diff)
        dy = np.diff(y_diff)
        x_slope_rad = np.arctan2(dx, 1)
        y_slope_rad = np.arctan2(dy, 1)
        x_slope_rad = np.concatenate(([0], x_slope_rad))
        y_slope_rad = np.concatenate(([0], y_slope_rad))
        x_slope_rad = savgol_filter(x_slope_rad, window_size, 2)
        y_slope_rad = savgol_filter(y_slope_rad, window_size, 2)
    else:
        x_slope_rad = np.zeros_like(x_diff)
        y_slope_rad = np.zeros_like(y_diff)
    ...
    max_x_slope = np.max(np.abs(x_slope_rad))
    max_y_slope = np.max(np.abs(y_slope_rad))
    avg_x_slope = np.mean(np.abs(x_slope_rad))
    avg_y_slope = np.mean(np.abs(y_slope_rad))
    ...
    x_slope_pct = ((max_x_slope - avg_x_slope) / avg_x_slope) * 100 if avg_x_slope != 0 else 0
    y_slope_pct = ((max_y_slope - avg_y_slope) / avg_y_slope) * 100 if avg_y_slope != 0 else 0
    ...
    print("\nSlope Analysis:")
    print(f"Maximum X slope: {max_x_slope:.4f} rad ({x_slope_pct:.1f}% above average)")
    print(f"Maximum Y slope: {max_y_slope:.4f} rad ({y_slope_pct:.1f}% above average)")
    print(f"Average X slope: {avg_x_slope:.4f} rad")
    print(f"Average Y slope: {avg_y_slope:.4f} rad")
    print("=================================")
    
    return {
        'x_difference': x_diff,
        'y_difference': y_diff,
        'x_slope_rad': x_slope_rad,
        'y_slope_rad': y_slope_rad,
        'frames': frames,
        'max_x_diff': max_x_diff,
        'max_y_diff': max_y_diff,
        'max_x_slope': max_x_slope,
        'max_y_slope': max_y_slope
    }
...
def plot_data(tracking_data, filtered_data, show_raw=False, absolute_diff=True):
    ...
    slope_colors = ['darkgreen', 'darkred']
    raw_alpha = 0.3 if show_raw else 0
    
    # Calculate differences and slopes
    diff_metrics = calculate_differences_and_slopes(filtered_data, absolute_diff)
    
    # [Previous plotting code remains exactly the same...]
    # Plot X Position
    ...
    # Plot Slopes in Radians
    ax6 = plt.subplot(3, 2, 6)
    if diff_metrics and 'x_slope_rad' in diff_metrics:
        ax6.plot(diff_metrics['frames'], diff_metrics['x_slope_rad'], '-', 
                color=slope_colors[0], label='X Slope (rad)')
        ax6.plot(diff_metrics['frames'], diff_metrics['y_slope_rad'], '-', 
                color=slope_colors[1], label='Y Slope (rad)')
        
        ax6.set_title('Slope of Differences (Radians)')
        ax6.set_xlabel('Frame number')
        ax6.set_ylabel('Slope (radians)')
        ax6.legend()
        ax6.grid(True)
        ax6.axhline(0, color='black', linestyle='--', alpha=0.5)
    else:
        ax6.text(0.5, 0.5, 'Slope data not available', 
                ha='center', va='center')
        ax6.set_title('Slopes (Not Available)')
