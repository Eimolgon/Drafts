...
def calculate_absolute_differences(filtered_data):
    """Calculate absolute differences between both ellipses."""
    ...
    x_diff = np.abs(np.array(filtered_data[0]['x_pos'][:min_frames]) - 
              np.array(filtered_data[1]['x_pos'][:min_frames]))
    y_diff = np.abs(np.array(filtered_data[0]['y_pos'][:min_frames]) - 
              np.array(filtered_data[1]['y_pos'][:min_frames]))
    
    return {
        'x_difference': x_diff,
        'y_difference': y_diff,
        'total_difference': np.sqrt(x_diff**2 + y_diff**2),  # Euclidean distance
        'frames': frames
    }

def plot_filtered_data_with_differences(filtered_data):
    """Plot filtered data with absolute differences."""
    plt.figure(figsize=(15, 12))
    
    # Define color scheme
    ellipse_colors = ['blue', 'orange']  # For the two ellipses
    diff_colors = ['green', 'red', 'purple']  # For difference plots
    
    # Calculate differences
    diff_metrics = calculate_absolute_differences(filtered_data)
    
    # Plot X Position
    ax1 = plt.subplot(3, 2, 1)
    for class_id, data in filtered_data.items():
        ax1.plot(data['frames'], data['x_pos'], '-', color=ellipse_colors[class_id], 
                linewidth=2, label=f'Ellipse {class_id} X')
    ax1.set_title('Filtered X Position')
    ...
    for class_id, data in filtered_data.items():
        ax2.plot(data['frames'], data['y_pos'], '-', color=ellipse_colors[class_id], 
                linewidth=2, label=f'Ellipse {class_id} Y')
    ax2.set_title('Filtered Y Position (Inverted)')
    ...
    for class_id, data in filtered_data.items():
        ax3.plot(data['frames'], data['major_axes'], '-', color=ellipse_colors[class_id], 
                linewidth=2, label=f'Ellipse {class_id}')
    ax3.set_title('Filtered Major Axis')
    ...
    if diff_metrics:
        ...
        ax5.plot(diff_metrics['frames'], diff_metrics['total_difference'], '-', 
                color=diff_colors[2], label='Euclidean Distance')
        ax5.set_title('Absolute Differences Between Ellipses')
        ax5.set_xlabel('Frame number')
        ax5.set_ylabel('Difference (pixels)')
        ax5.legend()
        ax5.grid(True)
    else:
        ax5.text(0.5, 0.5, 'Not enough ellipses for difference calculation', 
                ha='center', va='center')
        ax5.set_title('Absolute Differences (Not Available)')
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Plot filtered ellipse data with absolute differences.')
    ...
    plot_filtered_data_with_differences(filtered_data)
