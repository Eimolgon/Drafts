...
def eig_wobble(eigenvalue, phi_0):
    '''
    Function to determine if the mode is a wobble mode
    ...
    if phi_0 == 0:
        for i in eigenvalue:
            if i.imag > 35 and i.imag <= 60 and i.real > -14 and i.real < 0:
                wobble.append(i)
    elif phi_0 == np.radians(30):
        for i in eigenvalue:
            if i.imag > 50 and i.imag <= 70 and i.real > -50 and i.real < 15:
                wobble.append(i)
    elif phi_0 == np.radians(60):
        for i in eigenvalue:
            if i.imag > 60 and i.imag <= 85 and i.real > -50 and i.real < 0:
                wobble.append(i)
...
Cl = 2                    #Coeficiente de sustentación
...
V_i = 5
V_f = 101
Vx_rango = np.linspace(V_i, V_f, 200)
# Vx_rango = np.arange(V_i,V_f,10)
phi00 = [0, np.radians(30), np.radians(60)]
# lift_coef_iter = np.arange(0, 2.1, 0.5)
lift_coef_iter = [0.5, 1.0, 1.5, 2.0, 3.0]
CoP_pos_iter = [-0.6, 0, 0.6]

weave_supremo = []
wobble_supremo = []

dampingWeave_supremo = []
dampingWobble_supremo = []
...
for phi_0 in phi00:
    ...
    for j in CoP_pos_iter:
        ...
        for i in Vx_rango:
            ...
            A[0,5] = p_Yr2
            A[0,6] = p_Yf2
            A[0,7] = -DownForce*np.sin(phi_0) + X_f0*p_Delta1 + p_Yf5 + p_Yr3
            ...
            A[1,5] = p_Mr2
            A[1,6] = p_Mf2 + p_Yf2*w
            A[1,7] = a_x*h*m*np.cos(phi_0) -DownForce*b_A*np.sin(phi_0) + F_ad*h_A*np.sin(phi_0) - X_f0*rho_f*np.cos(phi_0) -\
        ...
        for k in A_total:
            ...
            if len(com1) > 0:
                weave.append(com1)
                dampingWeave.append(Damping_ratio(com1))

            com2 = eig_wobble(u_filtered, phi_0)
            if len(com2) > 0:
                wobble.append(com2)
                dampingWobble.append(Damping_ratio(com2))
            

        real_total.append(real_temporary)
        img_total.append(img_temporary)
        weave_total.append(weave)
        wobble_total.append(wobble)

        dampingWeave_total.append(dampingWeave)
        dampingWobble_total.append(dampingWobble)
        

    weave_supremo.append(weave_total)
    wobble_supremo.append(wobble_total)

    dampingWeave_supremo.append(dampingWeave_total)
    dampingWobble_supremo.append(dampingWobble_total)
    ...
    for ii in range(len(real_total)):
        ...
        sizecounter = 0
        for jj in range(0, len(real_total[ii]),4):
    ...
    black_circle = mlines.Line2D([], [], color = 'black', marker = 'o', \
                            linestyle = 'None', markersize = 10, fillstyle = 'none', \
                                label = r'$CoP = \textrm{Rear} (-0.6)$')
    black_square = mlines.Line2D([], [], color = 'black', marker = 's', \
                                linestyle = 'None', markersize = 10, fillstyle = 'none', \
                                    label = r'$CoP = \textrm{Aligned with CoM} (0)$')
    black_triangle = mlines.Line2D([], [], color = 'black', marker = '^', \
                                linestyle = 'None', markersize = 10, fillstyle = 'none', \
                                    label = r'$CoP = \textrm{Front} (+0.6)$')


    # ax.legend(handles = [black_tri, blue_circle, red_triangle, green_square, orange_diamond], loc = 'upper right')
    ax.legend(handles = [black_circle, black_square, black_triangle], loc = 'lower left', fontsize = 15)
    ax.axvline(x = 0, color = 'k', linestyle = '-')
    ax.set_xlabel('Real part [1/s]',size=15)
    ax.set_ylabel('Imaginary part [1/s]', size=15)
    ax.set_xlim([-50, 15])
    ax.set_ylim([-1, 90])
    ax.fill_betweenx(np.arange(-5, 100), 0, 40, where=(np.arange(-5, 100) >= -5), color='lightgray', alpha=0.5)


    ax.text(-35, 35, r'\textbf{Stable}', size=15)
    ax.text(2 , 35, r'\textbf{Unstable}', size=15)#, rotation = 'vertical')
    if phi_0 == 0:
        ax.text(-11, 60, r'\textit{wobble}', size=15)
        ax.text(-11, 11, r'\textit{weave}', size=15)

    elif phi_0 == np.radians(30):
        ax.text(-12, 69, r'\textit{wobble}', size=15)
        ax.text(-12, 10, r'\textit{weave}', size=15)

    elif phi_0 == np.radians(60):
        ax.text(-12, 75, r'\textit{wobble}', size=15)
        ax.text(-12, 11, r'\textit{weave}', size=15)
    ...
    plt.grid()
    if phi_0 == 0:
        plt.savefig('RL-phi0-pos.pdf')
    elif phi_0 == np.radians(30):
        plt.savefig('RL-phi30-pos.pdf')
    elif phi_0 == np.radians(60):
        plt.savefig('RL-phi60-pos.pdf')
    plt.show()



vx0_0 = np.linspace(V_i, V_f, len(weave_supremo[0][0]))
vx1_0 = np.linspace(V_i, V_f, len(weave_supremo[0][1]))
vx2_0 = np.linspace(V_i, V_f, len(weave_supremo[0][2]))

vx0_1 = np.linspace(V_i, V_f, len(weave_supremo[1][0]))
vx1_1 = np.linspace(V_i, V_f, len(weave_supremo[1][1]))
vx2_1 = np.linspace(V_i, V_f, len(weave_supremo[1][2]))

vx0_2 = np.linspace(V_i, V_f, len(weave_supremo[2][0]))
vx1_2 = np.linspace(V_i, V_f, len(weave_supremo[2][1]))
vx2_2 = np.linspace(V_i, V_f, len(weave_supremo[2][2]))


x_shadow = np.linspace(-5, 105, 100)
y_shadow = np.linspace(20, 20, 100)


plt.plot(vx0_0, weave_supremo[0][0], color = 'k', linestyle = 'solid', marker = 'o', fillstyle = 'none', markersize = 10, markevery = [-1])#, marker = '1', markersize = 12)
plt.plot(vx1_0, weave_supremo[0][1], color = 'k', linestyle = 'solid', marker = 's', fillstyle = 'none', markersize = 10, markevery = [-1])
plt.plot(vx2_0, weave_supremo[0][2], color = 'k', linestyle = 'solid', marker = '^', fillstyle = 'none', markersize = 10, markevery = [-1])

plt.plot(vx0_1, weave_supremo[1][0], color = 'k', linestyle = 'dashed', marker = 'o', fillstyle = 'none', markersize = 10, markevery = [80])#, marker = 's', fillstyle = 'none', markersize = 10, markevery = [0, -1])
plt.plot(vx1_1, weave_supremo[1][1], color = 'k', linestyle = 'dashed', marker = 's', fillstyle = 'none', markersize = 10, markevery = [109])
plt.plot(vx2_1, weave_supremo[1][2], color = 'k', linestyle = 'dashed', marker = '^', fillstyle = 'none', markersize = 10, markevery = [114])

plt.plot(vx0_2, weave_supremo[2][0], color = 'k', linestyle = 'dotted', marker = 'o', fillstyle = 'none', markersize = 10, markevery = [28])#, marker = 'o', fillstyle = 'none', markersize = 10, markevery = [0, -1])
plt.plot(vx1_2, weave_supremo[2][1], color = 'k', linestyle = 'dotted', marker = 's', fillstyle = 'none', markersize = 10, markevery = [28])
plt.plot(vx2_2, weave_supremo[2][2], color = 'k', linestyle = 'dotted', marker = '^', fillstyle = 'none', markersize = 10, markevery = [28])


weave_legendElements = [Line2D([0], [0], color = 'k', linestyle = 'solid',  label = r'$\varphi_0 = 0^{\circ}$'),
                        Line2D([0], [0], color = 'k', linestyle = 'dashed', label = r'$\varphi_0 = 30^{\circ}$'),
                        Line2D([0], [0], color = 'k', linestyle = 'dotted', label = r'$\varphi_0 = 60^{\circ}$'),
                        Line2D([0], [0], color = 'k', fillstyle = 'none', marker = 'o', label = r'$CoP = \textrm{Rear} (-0.6)$', linestyle = 'none', markersize = 10),
                        Line2D([0], [0], color = 'k', fillstyle = 'none', marker = 's', label = r'$CoP = \textrm{Aligned with CoM} (0.0)$', linestyle = 'none', markersize = 10),
                        Line2D([0], [0], color = 'k', fillstyle = 'none', marker = '^', label = r'$CoP = \textrm{Front} (+0.6)$', linestyle = 'none', markersize = 10)]
...
plt.text(10, -0.5, r'\textbf{Stable}', size=15)
plt.text(10, 0.5, r'\textbf{Unstable}', size=15)
plt.hlines(0, 0, 105, colors='k', linestyles='solid')
plt.legend(handles = weave_legendElements, loc = 'lower right', fontsize = 15)
plt.xlim([0, 105])
plt.ylim([-8.5, 4.1])
plt.grid()
plt.savefig('weave-pos.pdf')
plt.show()


fig, ax3 = plt.subplots()

vx0_3 = np.linspace(V_i, V_f, len(wobble_supremo[0][0]))
vx1_3 = np.linspace(V_i, V_f, len(wobble_supremo[0][1]))
vx2_3 = np.linspace(V_i, V_f, len(wobble_supremo[0][2]))

vx0_4 = np.linspace(V_i, V_f, len(wobble_supremo[1][0]))
vx1_4 = np.linspace(V_i, V_f, len(wobble_supremo[1][1]))
vx2_4 = np.linspace(V_i, V_f, len(wobble_supremo[1][2]))

vx0_5 = np.linspace(V_i, V_f, len(wobble_supremo[2][0]))
vx1_5 = np.linspace(V_i, V_f, len(wobble_supremo[2][1]))
vx2_5 = np.linspace(V_i, V_f, len(wobble_supremo[2][2]))



ax3.plot(vx0_3, wobble_supremo[0][0], color = 'k', linestyle = 'solid', marker = 'o', fillstyle = 'none', markersize = 10, markevery = [-1])
ax3.plot(vx1_3, wobble_supremo[0][1], color = 'k', linestyle = 'solid', marker = 's', fillstyle = 'none', markersize = 10, markevery = [-1])
ax3.plot(vx2_3, wobble_supremo[0][2], color = 'k', linestyle = 'solid', marker = '^', fillstyle = 'none', markersize = 10, markevery = [-1])

ax3.plot(vx0_4, wobble_supremo[1][0], color = 'k', linestyle = 'dashed', marker = 'o', fillstyle = 'none', markersize = 10, markevery = [59])
ax3.plot(vx1_4, wobble_supremo[1][1], color = 'k', linestyle = 'dashed', marker = 's', fillstyle = 'none', markersize = 10, markevery = [69])
ax3.plot(vx2_4, wobble_supremo[1][2], color = 'k', linestyle = 'dashed', marker = '^', fillstyle = 'none', markersize = 10, markevery = [83])

ax3.plot(vx0_5, wobble_supremo[2][0], color = 'k', linestyle = 'dotted', marker = 'o', fillstyle = 'none', markersize = 10, markevery = [24])
ax3.plot(vx1_5, wobble_supremo[2][1], color = 'k', linestyle = 'dotted', marker = 's', fillstyle = 'none', markersize = 10, markevery = [30])
ax3.plot(vx2_5, wobble_supremo[2][2], color = 'k', linestyle = 'dotted', marker = '^', fillstyle = 'none', markersize = 10, markevery = [35])
# fig.set_figheight(9)
# fig.set_figwidth(16)
...
plt.text(5, -7, r'\textbf{Stable}', size=15)
plt.text(5, 5, r'\textbf{Unstable}', size=15)
plt.hlines(0, 0, 105, colors='k', linestyles='solid')
# plt.legend(loc = 'lower right', fontsize = '15')
plt.legend(handles = weave_legendElements, loc = 'lower left', fontsize = 15)
plt.xlim([0, 105])
plt.ylim([-50, 16])


xx1, xx2, yy1, yy2 = 30, 37, -17.5, -11
axins1 = ax3.inset_axes([0.5, 0.7, 0.25, 0.25],
    xlim=(xx1, xx2), ylim=(yy1, yy2))

axins1.plot(vx0_5, wobble_supremo[2][0], color = 'k', linestyle = 'dotted', marker = 'o', fillstyle = 'none', markersize = 10, markevery = [24])
axins1.plot(vx1_5, wobble_supremo[2][1], color = 'k', linestyle = 'dotted', marker = 's', fillstyle = 'none', markersize = 10, markevery = [30])
axins1.plot(vx2_5, wobble_supremo[2][2], color = 'k', linestyle = 'dotted', marker = '^', fillstyle = 'none', markersize = 10, markevery = [35])
axins1.fill_between(x_shadow, y_shadow, color='lightgray', alpha = 0.5)
axins1.hlines(0, 0, 105, colors='k', linestyles='solid')
axins1.grid()
ax3.indicate_inset_zoom(axins1, edgecolor="black")


plt.grid()
plt.savefig('wobble-pos.pdf')
# plt.show()
...
plt.plot(vx0_0, dampingWeave_supremo[0][0], color = 'k', linestyle = 'solid', marker = 'o', fillstyle = 'none', markersize = 10, markevery = [-1])#, marker = '1', markersize = 12)
plt.plot(vx1_0, dampingWeave_supremo[0][1], color = 'k', linestyle = 'solid', marker = 's', fillstyle = 'none', markersize = 10, markevery = [-1])
plt.plot(vx2_0, dampingWeave_supremo[0][2], color = 'k', linestyle = 'solid', marker = '^', fillstyle = 'none', markersize = 10, markevery = [-1])

plt.plot(vx0_1, dampingWeave_supremo[1][0], color = 'k', linestyle = 'dashed', marker = 'o', fillstyle = 'none', markersize = 10, markevery = [80])#, marker = 's', fillstyle = 'none', markersize = 10, markevery = [0, -1])
plt.plot(vx1_1, dampingWeave_supremo[1][1], color = 'k', linestyle = 'dashed', marker = 's', fillstyle = 'none', markersize = 10, markevery = [109])
plt.plot(vx2_1, dampingWeave_supremo[1][2], color = 'k', linestyle = 'dashed', marker = '^', fillstyle = 'none', markersize = 10, markevery = [114])

plt.plot(vx0_2, dampingWeave_supremo[2][0], color = 'k', linestyle = 'dotted', marker = 'o', fillstyle = 'none', markersize = 10, markevery = [28])#, marker = 'o', fillstyle = 'none', markersize = 10, markevery = [0, -1])
plt.plot(vx1_2, dampingWeave_supremo[2][1], color = 'k', linestyle = 'dotted', marker = 's', fillstyle = 'none', markersize = 10, markevery = [28])
plt.plot(vx2_2, dampingWeave_supremo[2][2], color = 'k', linestyle = 'dotted', marker = '^', fillstyle = 'none', markersize = 10, markevery = [28])
...
plt.plot(vx0_3, dampingWobble_supremo[0][0], color = 'k', linestyle = 'solid', marker = 'o', fillstyle = 'none', markersize = 10, markevery = [-1])
plt.plot(vx1_3, dampingWobble_supremo[0][1], color = 'k', linestyle = 'solid', marker = 's', fillstyle = 'none', markersize = 10, markevery = [-1])
plt.plot(vx2_3, dampingWobble_supremo[0][2], color = 'k', linestyle = 'solid', marker = '^', fillstyle = 'none', markersize = 10, markevery = [-1])

plt.plot(vx0_4, dampingWobble_supremo[1][0], color = 'k', linestyle = 'dashed', marker = 'o', fillstyle = 'none', markersize = 10, markevery = [59])
plt.plot(vx1_4, dampingWobble_supremo[1][1], color = 'k', linestyle = 'dashed', marker = 's', fillstyle = 'none', markersize = 10, markevery = [69])
plt.plot(vx2_4, dampingWobble_supremo[1][2], color = 'k', linestyle = 'dashed', marker = '^', fillstyle = 'none', markersize = 10, markevery = [83])

plt.plot(vx0_5, dampingWobble_supremo[2][0], color = 'k', linestyle = 'dotted', marker = 'o', fillstyle = 'none', markersize = 10, markevery = [24])
plt.plot(vx1_5, dampingWobble_supremo[2][1], color = 'k', linestyle = 'dotted', marker = 's', fillstyle = 'none', markersize = 10, markevery = [30])
plt.plot(vx2_5, dampingWobble_supremo[2][2], color = 'k', linestyle = 'dotted', marker = '^', fillstyle = 'none', markersize = 10, markevery = [35])
...
plt.text(5, 5, r'\textbf{Unstable}', size=15)
plt.hlines(0, 0, 105, colors='k', linestyles='solid')
plt.legend(handles = weave_legendElements, loc = 'upper left', fontsize = 15)
plt.xlim([0, 105])
plt.ylim([-0.5, 0.8])
plt.grid()
plt.show()
...
# plt.plot(vx0, weave_total[0], color = 'k', linestyle = 'dashed', label = 'CoP = -0.6')
# plt.plot(vx1, weave_total[1], color = 'k', linestyle = 'solid', label = 'CoP = 0')
# plt.plot(vx2, weave_total[2], color = 'k', linestyle = 'dashdot', label = 'CoP = +0.6')
...
# ax2.plot(vx0_0, weave_supremo[0][0], color = 'k', linestyle = 'dashed')#, marker = '1', markersize = 12)
# ax2.plot(vx1_0, weave_supremo[0][1], color = 'k', linestyle = 'solid')#, marker = '1', markersize = 12)
# ax2.plot(vx2_0, weave_supremo[0][2], color = 'k', linestyle = 'dashdot')#, marker = '1', markersize = 12)

# ax2.plot(vx0_1, weave_supremo[1][0], color = 'k', linestyle = 'dashed', marker = '1', markersize = 12)
# ax2.plot(vx1_1, weave_supremo[1][1], color = 'k', linestyle = 'solid', marker = '1', markersize = 12)
# ax2.plot(vx2_1, weave_supremo[1][2], color = 'k', linestyle = 'dashdot', marker = '1', markersize = 12)

# ax2.plot(vx0_2, weave_supremo[2][0], color = 'k', linestyle = 'dashed', label = r'$C_l = 0.0$', marker = 'o', fillstyle = 'none', markersize = 12)
# ax2.plot(vx1_2, weave_supremo[2][1], color = 'k', linestyle = 'solid', label = r'$C_l = 1.0$', marker = 'o', fillstyle = 'none', markersize = 12)
# ax2.plot(vx2_2, weave_supremo[2][2], color = 'k', linestyle = 'dashdot', label = r'$C_l = 2.0$', marker = 'o', fillstyle = 'none', markersize = 12)




# # ax2.plot(vx0_0, weave_supremo[0][0], color = 'k', linestyle = 'solid', marker = '1', markersize = 12)
# # ax2.plot(vx1_0, weave_supremo[0][1], color = 'k', linestyle = 'solid', marker = 'o', fillstyle = 'none', markersize = 12)
# # ax2.plot(vx2_0, weave_supremo[0][2], color = 'k', linestyle = 'solid', marker = '^', fillstyle = 'none', markersize = 12)

# # ax2.plot(vx0_1, weave_supremo[1][0], color = 'k', linestyle = 'dashed', marker = '1', markersize = 12)
# # ax2.plot(vx1_1, weave_supremo[1][1], color = 'k', linestyle = 'dashed', marker = 'o', fillstyle = 'none', markersize = 12)
# # ax2.plot(vx2_1, weave_supremo[1][2], color = 'k', linestyle = 'dashed', marker = '^', fillstyle = 'none', markersize = 12)

# # ax2.plot(vx0_2, weave_supremo[2][0], color = 'k', linestyle = 'dashdot', label = r'$C_l = 0.0$', marker = '1')#, fillstyle = 'none', markersize = 12)
# # ax2.plot(vx1_2, weave_supremo[2][1], color = 'k', linestyle = 'dashdot', label = r'$C_l = 1.0$', marker = 'o', fillstyle = 'none', markersize = 12)
# # ax2.plot(vx2_2, weave_supremo[2][2], color = 'k', linestyle = 'dashdot', label = r'$C_l = 2.0$', marker = '^', fillstyle = 'none', markersize = 12)
...
# xx1, xx2, yy1, yy2 = 38, 63, -0.5, 0.5
# axins = ax2.inset_axes([0.4, 0.125, 0.2, 0.3],
#     xlim=(xx1, xx2), ylim=(yy1, yy2))

# axins.plot(vx0_1, weave_supremo[1][0], color = 'k', linestyle = 'dashed', marker = '1', markersize = 12, label = 'CoP = Rear')
# axins.plot(vx1_1, weave_supremo[1][1], color = 'k', linestyle = 'solid', marker = '1', markersize = 12, label = 'CoP = CoM')
# axins.plot(vx2_1, weave_supremo[1][2], color = 'k', linestyle = 'dashdot', marker = '1', markersize = 12, label = 'CoP = Front')
...
# xx1_2, xx2_2, yy1_2, yy2_2 = 72, 80, -0.25, 0.25
# axins_2 = ax2.inset_axes([0.7, 0.125, 0.2, 0.3],
#     xlim=(xx1_2, xx2_2), ylim=(yy1_2, yy2_2))

# axins_2.plot(vx0_0, weave_supremo[0][0], color = 'k', linestyle = 'dashed', label = 'CoP = Rear')
# axins_2.plot(vx1_0, weave_supremo[0][1], color = 'k', linestyle = 'solid', label = 'CoP = CoM')
# axins_2.plot(vx2_0, weave_supremo[0][2], color = 'k', linestyle = 'dashdot', label = 'CoP = Front')
# axins_2.fill_between(x_shadow, y_shadow, color='lightgray', alpha = 0.5)
# axins_2.hlines(0, 0, 105, colors='k', linestyles='solid')
# axins_2.grid()
# ax2.indicate_inset_zoom(axins_2, edgecolor="black")
